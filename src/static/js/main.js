document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!chatThreadId) {
        // CORREGIDO: Usar 'crypto' directamente en lugar de 'window.crypto'
        if (typeof crypto !== 'undefined' && crypto.randomUUID) {
            ...
            chatThreadId = `fallback-thread-id-${Date.now()}${randomPart}`;
            localStorage.setItem('chatThreadId', chatThreadId);
    ...
    async function handleChatSubmit() {
        ...
        let systemMessageContentDiv = null;
        ...
        try {
            ...
            systemMessageContentDiv = addMessageToChat("", 'system');
    ...
    let currentSearchParamsForPagination = {};

    // CORREGIDO: Renombrado el parámetro no usado 'pageRequest' a '_pageRequest'
    function mostrarResultados(data, _pageRequest) {
        ...
        // CORREGIDO: Eliminada la variable no usada 'pageSizeApi'
        const totalPages = data.totalPages;
        const currentPageApi = data.number;
    ...
    async function mostrarDetallesConvocatoria(id) {
        ...
        try {
            ...
            const convocatoria = (Array.isArray(data) && data.length > 0) ? data[0] : 
                                 ...
                                 (data && Object.keys(data).length > 0 && !Array.isArray(data)) ? data :
                                 null;
            ...
            const fechaPublicacion = formatFecha(convocatoria.fechaPublicacion || convocatoria.fechaRegistroDesdeSolicitingToolDate || convocatoria.fechaCreacionBDNS);
            htmlDetalles += `<p class="mb-1"><strong>Publicación:</strong> ${fechaPublicacion || 'No especificada'}</p>`;
