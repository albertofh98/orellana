...
class BeneficiariesAgent:
    ...
    def get_beneficiaries_by_year(self, state: GraphState) -> dict:
        ...
        try:
            ...
            logger.info(f"{node_name}: Respuesta CRUDA de la API (tipo: {type(api_response_object)}): {str(api_response_object)[:1000]}")
            ...
            if isinstance(api_response_object, dict) and 'content' in api_response_object:
                ...
                if isinstance(beneficiaries_list_from_api, list):
                    if not beneficiaries_list_from_api:
                        logger.info(
                            f"{node_name}: La API devolvió una lista 'content' vacía para los años {target_years_int_list}."
                        )
                    ...
                    for beneficiary_item in beneficiaries_list_from_api:
                        ...
                        if item_year_field is None:
                            logger.warning(
                                f"{node_name}: Beneficiario en 'content' sin campo 'ejercicio': {str(beneficiary_item)[:100]}"
                            )
                            continue
                        ...
                        try:
                            ...
                            if item_year_int in structured_api_data_for_prompt:
                                ...
                                logger.warning(
                                    f"{node_name}: API devolvió en 'content' datos para el año {item_year_int} que no fue solicitado o no está en target_years_int_list. Datos: {beneficiary_item}"
                                )
                        except ValueError:
                            logger.warning(
                                f"{node_name}: Valor de 'ejercicio' no entero ('{item_year_field}') en item de beneficiario: {beneficiary_item}"
                            )
                else:
                    logger.warning(
                        f"{node_name}: La clave 'content' en la respuesta de la API no es una lista. Tipo: {type(beneficiaries_list_from_api)}"
                    )
                ...
                for beneficiary_item in api_response_object:
                    ...
                    if item_year_field is not None:
                        try:
                            ...
                            logger.warning(
                                f"{node_name}: Valor de 'ejercicio' no entero ('{item_year_field}') en item de beneficiario (lista directa): {beneficiary_item}"
                            )
            else:
                logger.warning(
                    f"{node_name}: Respuesta inesperada de la API. No es un diccionario con 'content' ni una lista. Se asumirá que no hay datos. Tipo: {type(api_response_object)}"
                )
        ...
        logger.info(
            f"{node_name}: Datos estructurados que se pasarán al generador (api_response_data): {json.dumps(structured_api_data_for_prompt, ensure_ascii=False, indent=2)}"
        )
