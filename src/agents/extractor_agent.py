...
class ExtractorAgent:
    ...
    def extract_search_params(self, state: GraphState) -> dict:
        ...
        if not parsed_llm_json:
            ...
            # y son válidas (el LLM debe devolver formato DD/MM/YYYY)
            ...
            # Si la descripción está vacía después de todo,
            # podríamos usar la consulta original como fallback,
            ...
            if not api_params_to_send['descripcion'] and query:
                ...
                api_params_to_send['descripcion'] = query
            if not api_params_to_send['descripcion']:
        ...
        if error_msg and not api_params_to_send:
        ...
        return {
            "api_call_params": api_params_to_send,
            "error_message": error_msg,
            "last_stream_event_node": node_name
        }
    ...
    def extract_party_params(self, state: GraphState) -> dict:
        ...
        query = state['original_query']
        logger.info(f"Nodo: {node_name}, extrayendo parámetros para buscar partido de: '{query}'")
        prompt = self.prompts['extract_party_params'] \
            .replace('ORIGINAL_QUERY', query)
        params_text = generate_content_non_stream(self._model, prompt)
        parsed_json = parse_json_from_text(params_text, default_if_error={})
        logger.info(f"Texto de parámetros extraídos del LLM (extract_party_params): {parsed_json}")

        if not parsed_json or not parsed_json.get("beneficiario"):
            return {
                "error_message": "No pude identificar el nombre del partido en tu consulta.",
                "last_stream_event_node": node_name
            }
        return {
            "api_call_params": {
