import logging
import json
# from typing: Ya no se usa directamente aquí para la salida de los helpers
...
class GeneratorAgent:
    ...
    def generate_parties_summary(self, state: GraphState) -> dict:
        ...
        data_json = json.dumps(parties_data, ensure_ascii=False, indent=2) if parties_data else "{}"

        replacements = {
            "ORIGINAL_QUERY": state['original_query'],
            "PARTIES_DATA_JSON": data_json
        }
        return self._prepare_response_state(state, 'parties_summary', node_name, replacements)
